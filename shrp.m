function [f0_time,f0_value,T0, SHR,f0_candidates]=shrp(Y,Fs,F0MinMax,frame_length,timestep,SHR_Threshold,ceiling,med_smooth,CHECK_VOICING)
...
for loop = 1: length(f0_value)
    
    pitch = f0_value(loop);
    if pitch ~= 0
        T0(loop) = 1/pitch;
        f0_value(loop) = round(pitch);
    else
        T0(loop) = 0;
        f0_vlaue(loop) = 0;
    end
